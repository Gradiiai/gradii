...
export default function CodingInterviewPage({
  interviewId}: CodingInterviewPageProps) {
  // State management
    ...
    const fetchExistingAnswer = async () => {
      ...
      try {
        ...
        if (existingAnswers.length > 0) {
          ...
          // Extract code from feedback JSON if available
          if (answer.feedback) {
            try {
              const parsedFeedback = answer.feedback ? JSON.parse(answer.feedback) : {};
              setCode(parsedFeedback.userAnswer || getInitialCode(language));
  ...
  const fetchInterviewData = useCallback(async () => {
    ...
    try {
      ...
      if (result.length > 0) {
        ...
        try {
          const codingQuestionsData = JSON.parse(interviewData.codingQuestions);

          if (
            Array.isArray(codingQuestionsData) &&
            codingQuestionsData.length > 0
          ) {
            setInterviewData(interviewData);
            setCodingInterviewQuestions(codingQuestionsData);

            // timeLimit is now a number representing minutes
            setInterviewDuration(interviewData.timeLimit);
          } else {
  ...
  const handleCompileAndRun = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/coding/compile", {
        method: "POST",
        ...
        <div className="bg-white border-b border-gray-200 p-4 shadow-sm">
          <div className="flex items-center gap-4 flex-wrap">
            <div className="flex items-center gap-2 bg-gray-50 rounded-lg p-2">
              <Select
                onValueChange={(value: "java" | "cpp" | "python" | "php") => {
                  setLanguage(value);
...
function getInitialCode(language: "java" | "cpp" | "python" | "php") {
  if (language === "java") {
