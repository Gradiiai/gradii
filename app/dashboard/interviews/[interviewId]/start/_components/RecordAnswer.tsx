    ...
    const fetchExistingAnswer = async () => {
      ...
      try {
        ...
        if (existingAnswers.length > 0) {
          // Find the answer for the current question by parsing feedback
          const currentQuestion = interviewQuestions[activeQuestionIndex].question || interviewQuestions[activeQuestionIndex].Question || '';
          const matchingAnswer = existingAnswers.find(answer => {
            try {
              const feedback = JSON.parse(answer.feedback || '{}');
              return feedback.question === currentQuestion;
            } catch {
              return false;
            }
          });
          ...
          if (matchingAnswer) {
            try {
              const feedback = JSON.parse(matchingAnswer.feedback || '{}');
              setExistingAnswer(matchingAnswer);
              setTranscript(feedback.userAnswer || '');
              setFeedback({
                rating: matchingAnswer.score || 0,
  ...
  const saveUserAnswer = async () => {
    ...
    try {
      const response = await fetch("/api/ai/generate-feedback", {
        ...
        body: JSON.stringify({
          question: interviewQuestions[activeQuestionIndex].question || interviewQuestions[activeQuestionIndex].Question,
          answer: transcript})});
      ...
      const answerData = {
        ...
        feedback: JSON.stringify({
          [`question_${activeQuestionIndex}`]: {
            question: interviewQuestions[activeQuestionIndex].question || interviewQuestions[activeQuestionIndex].Question || '',
            correctAnswer: interviewQuestions[activeQuestionIndex].correctAnswer || interviewQuestions[activeQuestionIndex].Answer || '',
      ...
      try {
        await checkAndAutoCompleteInterview(
          interviewData.interviewId,
          'behavioral',
          session?.user?.id ?? ''
        );
