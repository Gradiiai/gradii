...
import { JobDetailsForm, JobDescriptionTemplate } from "@/shared/types/job-campaign";
...
const jobDetailsSchema = z.object({
  campaignName: z.string().min(3, "Campaign name must be at least 3 characters").max(100, "Campaign name too long"),
  ...
  numberOfOpenings: z.number().min(1, "At least 1 opening required").max(100, "Too many openings"),
  salaryMin: z.number().optional(),
  salaryMax: z.number().optional(),
  jobDescription: z.string().min(50, "Job description must be at least 50 characters").max(5000, "Job description too long"),
  ...
  targetHireDate: z.string().optional()}).refine((data) => {
...
export default function JobDetailsStep() {
  ...
  const [jobDescriptionTemplates, setJobDescriptionTemplates] = useState<JobDescriptionTemplate[]>([]);
...
function formatDateInput(dateString: string | null): string {
  ...
  const validateForm = () => {
    try {
      const formData = {
        ...
        numberOfOpenings: parseInt(state.jobDetails.numberOfOpenings?.toString() || '1'),
        salaryMin: state.jobDetails.salaryMin ? parseInt(state.jobDetails.salaryMin.toString()) : undefined,
        salaryMax: state.jobDetails.salaryMax ? parseInt(state.jobDetails.salaryMax.toString()) : undefined,
  ...
  const saveJobCampaign = async () => {
    ...
    try {
      ...
      const url = isEditMode && state.campaignId 
      ...
      if (result.success && result.data) {
        ...
        toast.success(isEditMode ? 'Job campaign updated successfully!' : 'Job campaign created successfully!');
        return true;
      } else {
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to save job campaign');
      return false;
    } finally {
      setIsSaving(false);
      setSaveProgress(0);
    ...
    try {
      ...
      if (data.success && data.jobDuties) {
        ...
        const formattedJobDescription = Array.isArray(data.jobDuties) 
          ? data.jobDuties.map((duty: string, index: number) => `${index + 1}. ${duty}`).join('\n')
          : data.jobDuties;
        
        updateJobDetails({ jobDescription: formattedJobDescription });
        toast.success(`Job duties generated successfully! (${data.generated === 'ai' ? 'AI-powered' : 'Template-based'})`);
      } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const campaignData = {
         ...
         jobDescriptionTemplateId: state.jobDetails.jobDescriptionTemplateId || null,
         skillTemplateId: state.jobDetails.skillTemplateId || null,
         applicationDeadline: state.jobDetails.applicationDeadline ? new Date(state.jobDetails.applicationDeadline) : null,
         targetHireDate: state.jobDetails.targetHireDate ? new Date(state.jobDetails.targetHireDate) : null,
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Create Job Campaign</h1>
              <p className="text-gray-600">Step 1 of 5: Job Details</p>
            </div>
          ...
          {isSaving && (
            <div className="mt-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm text-gray-600">Saving campaign... {saveProgress}%</span>
              </div>
            ...
            <CardContent className="space-y-6">
              ...
              {companyName && (
                <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                  <div className="flex items-center gap-2">
                    <Briefcase className="w-5 h-5 text-blue-600" />
                    <span className="text-sm font-medium text-blue-900">Company: {companyName}</span>
                    {loadingCompany && <Loader2 className="w-4 h-4 animate-spin text-blue-600" />}
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <Select value={state.jobDetails.department || ''} onValueChange={(value) => handleInputChange('department', value)}>
                    <SelectTrigger className={`h-11 ${
              ...
              {/* Work Arrangement */}
              <div className="space-y-4">
                <h4 className="text-md font-medium text-gray-900">Work Arrangement</h4>
                <div className="flex items-center gap-6">
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <Label htmlFor="isRemote" className="text-sm font-medium">Remote Work</Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Switch
                      id="isHybrid"
                    ...
                    />
                    <Label htmlFor="isHybrid" className="text-sm font-medium">Hybrid Work</Label>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <Select value={state.jobDetails.experienceLevel || ''} onValueChange={(value) => handleInputChange('experienceLevel', value)}>
                    <SelectTrigger className={`h-11 ${
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={state.jobDetails.employeeType || ''} onValueChange={(value) => handleInputChange('employeeType', value)}>
                      <SelectTrigger className={`h-11 ${
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="space-y-2">
                    ...
                    <Select value={state.jobDetails.currency || 'INR'} onValueChange={(value) => handleInputChange('currency', value)}>
                      <SelectTrigger className="h-11">
                  ...
                  <div className="space-y-2">
                    <Label className="text-sm font-medium">Minimum Salary ({state.jobDetails.currency})</Label>
                    <Input
                      type="number"
                  ...
                  <div className="space-y-2">
                    <Label className="text-sm font-medium">Maximum Salary ({state.jobDetails.currency})</Label>
                    <Input
                      type="number"
                ...
                <div className="flex items-center space-x-2 pt-2">
                  <Switch
                    ...
                    onCheckedChange={(checked) => handleInputChange('salaryNegotiable', checked)}
                  />
                  <Label htmlFor="salaryNegotiable" className="text-sm font-medium">
                        ...
                        {(state.jobDetails.skills || []).map((skill, index) => (
                          <Badge key={index} variant="secondary" className="flex items-center gap-1">
                            ...
                            <X 
                              ...
                              onClick={() => handleInputChange('skills', (state.jobDetails.skills || []).filter(s => s !== skill))}
                            />
                          </Badge>
                        ))}
...
<Separator />
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    <Label className="text-sm font-medium">Job Description Template</Label>
                    <Select value={selectedTemplate} onValueChange={(value) => {
                      setSelectedTemplate(value);
                      const template = jobDescriptionTemplates.find(t => t.id === value);
                      if (template && template.templateContent) {
                        updateJobDetails({ jobDescription: template.templateContent });
                      }
                    }}>
                      <SelectTrigger className="h-11">
                    ...
                    {showTemplatePreview && selectedTemplate && selectedTemplate !== 'custom' && (
                      <div className="p-3 bg-gray-50 rounded-lg border">
                        <p className="text-sm text-gray-600">
                          {jobDescriptionTemplates.find(t => t.id === selectedTemplate)?.templateContent || 'Template preview not available'}
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <Label className="text-sm font-medium">Job Duties & Responsibilities</Label>
                      <div className="flex items-center gap-2">
                        <Button 
                          variant="outline" 
                          size="sm"
